...
async function buildResponsesPayload(options, attachments) {
	const instructions = [options.system?.trim(), ...options.messages.filter((message) => message.role === "system").map((message) => blocksToText(message.content).trim())].filter((value) => Boolean(value));
	const input = [];
	const knownToolCalls = /* @__PURE__ */ new Set();
	for (const message of options.messages) {
		if (message.role === "system") continue;
		const replayItems = replayOutputItems(message);
		if (message.role === "assistant" && replayItems !== null) {
			input.push(...replayItems);
			for (const item of replayItems) if (item.type === "function_call" && typeof item.call_id === "string") knownToolCalls.add(item.call_id);
		...
		if (toolResult?.type === "tool-result") {
			...
			if (knownToolCalls.has(callId)) input.push({
...
var ResponsesClient = class {
	...
	async *stream(options) {
		...
		try {
			yield* parseResponsesStream(await this.send(payload, sessionId, options.signal), options.signal);
		} finally {
...
async function* parseResponsesStream(response, signal) {
