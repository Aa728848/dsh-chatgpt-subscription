...
function resolveCodexModel(model, preferences) {
	...
	const effectiveDefaultEffort = customEffort && efforts.includes(customEffort) ? customEffort : entry.defaultReasoningEffort;
	return {
		provider: PROVIDER_ID,
		id: model,
...
function readBase64Image(value) {
	const root = record$3(value);
	const data = Array.isArray(root?.data) ? root.data[0] : null;
	const image = string$1(record$3(data)?.b64_json) ?? string$1(record$3(data)?.image_base64) ?? string$1(root?.b64_json) ?? string$1(root?.image);
...
var SettingsPreferenceStore = class {
	...
	async update(patch) {
		...
		if (patch.subagentModelEfforts !== void 0) normalized.subagentModelEfforts = {
...
async function listAllProviderGroups(ctx) {
	try {
		...
		for (const provider of providers) try {
			...
			for (const model of models) try {
				const efforts = (await ctx.llm?.resolveModelInfo?.(provider.id, model.id))?.reasoning?.efforts?.map((e) => String(e.id)) ?? [];
				modelEntries.push({
					id: model.id,
			...
			if (modelEntries.length > 0) groups.push({
				id: provider.id,
...
async function restoreDefaultWebProviders(loader) {
	for (const entry of loader.entries()) if (entry.options.id === "web" || entry.options.name === "@deepseek-ai/dsh-web") {
		...
		if (changed) await entry.update({ config: nextConfig }, true);
	}
}
...
function apply(ctx) {
	...
	restoreDefaultWebProviders(ctx.loader).catch(() => void 0);
	ctx.effect(() => {
		...
		if (origResolveCallConfig) ctx.llm.resolveCallConfig = async (options, signal) => {
			const status = preferences.status();
			const modelKey = `${options.provider}/${options.model}`;
			const customEffort = status.subagentModelEfforts?.[modelKey] ?? (options.provider === PROVIDER_ID ? status.subagentReasoningEffort : void 0);
			if (options.reasoningEffort === void 0 && customEffort) options = {
				...options,
				reasoningEffort: customEffort
			};
			return origResolveCallConfig(options, signal);
		};
		if (origResolveModelInfo) ctx.llm.resolveModelInfo = async (provider, model) => {
			const info = await origResolveModelInfo(provider, model);
			const status = preferences.status();
			const modelKey = `${provider}/${model}`;
			const customEffort = status.subagentModelEfforts?.[modelKey] ?? (provider === PROVIDER_ID ? status.subagentReasoningEffort : void 0);
			if (customEffort && info?.reasoning && info.reasoning.efforts.some((e) => e.id === customEffort)) return {
				...info,
				reasoning: {
					...info.reasoning,
					defaultEffort: customEffort
				}
			};
			return info;
		};
...
//#endregion
export { CodexChatGptAdapter, LinuxFileTokenStore, MacKeychainTokenStore, OAuthService, ProxyManager, ResponsesClient, UsageService, WindowsDpapiTokenStore, apply, createCodexImageTool, createPlatformTokenStore, detectSystemProxy, inject, mapCodexUsage, parseCodexUsage, parseResponsesStream };
