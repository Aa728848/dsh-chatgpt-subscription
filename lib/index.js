...
const DEFAULT_MAX_BODY_LENGTH = 2 * 1024 * 1024;
function createCodexFetchProvider(options = {}) {
	...
	return {
		...
		async fetch(request, signal) {
			...
			if (!isHttpUrl$1(targetUrl)) throw new WebError(`Invalid or unsupported URL: "${targetUrl}"`, "WEB_PROVIDER_INVALID_URL");
			let response;
			try {
				...
				if (signal?.aborted || error instanceof Error && error.name === "AbortError") throw new WebError("Web fetch request was aborted.", "WEB_PROVIDER_CANCELLED", { cause: error });
			...
			const kind = contentType.includes("text/html") || contentType.includes("application/xhtml+xml") ? "html" : "text";
			let rawText = "";
			try {
				rawText = await response.text();
			} catch (error) {
				if (signal?.aborted || error instanceof Error && error.name === "AbortError") throw new WebError("Web fetch body reading was aborted.", "WEB_PROVIDER_CANCELLED", { cause: error });
			...
			return {
				...
				body: {
					kind,
					content: truncated ? rawText.slice(0, maxBodyLength) : rawText
				},
				truncated
			};
		}
