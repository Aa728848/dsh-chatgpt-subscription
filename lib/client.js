window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var SubscriptionApi = class {
			...
			async providers() {
				const status = await request(`${ROUTE_PREFIX}/providers`);
			...
			importProviderCandidate(providerId, candidateId) {
				return this.providerPost(`${ROUTE_PREFIX}/providers/candidate/import`, {
					providerId,
					candidateId
				});
			}
			...
			pollProviderLogin(providerId, sessionId) {
				return this.providerPost(`${ROUTE_PREFIX}/providers/login/poll`, {
					providerId,
					sessionId
				});
			}
			submitProviderCode(providerId, sessionId, code) {
				return this.providerPost(`${ROUTE_PREFIX}/providers/login/code`, {
					providerId,
					sessionId,
					code
				});
			}
			cancelProviderLogin(providerId, sessionId) {
				return this.providerPost(`${ROUTE_PREFIX}/providers/login/cancel`, {
					providerId,
					sessionId
				});
			}
			...
			removeProviderAccount(providerId, accountId) {
				return this.providerPost(`${ROUTE_PREFIX}/providers/account/remove`, {
					providerId,
					accountId
				});
			}
			async providerPost(url, body) {
				if (!this.providerCsrfToken) await this.providers();
				return post(url, body, { "x-dsh-csrf-token": this.providerCsrfToken });
			}
		};
		...
		//#endregion
		//#region src/client/ProviderSubscriptionsSection.tsx
		const api = new SubscriptionApi();
		function ProviderSubscriptionsSection({ embedded = false } = {}) {
			const [status, setStatus] = (0, react.useState)(null);
			const [busy, setBusy] = (0, react.useState)(null);
			const [message, setMessage] = (0, react.useState)("");
			const [pending, setPending] = (0, react.useState)({});
			const [codes, setCodes] = (0, react.useState)({});
			const loginPopups = (0, react.useRef)({});
			const load = async () => {
			...
			const login = async (providerId) => {
				...
				try {
					...
					if (auth.status === "completed") setMessage(`${providerId} 登录完成`);
					else if (auth.instructions) setMessage(auth.instructions);
				} catch (error) {
					popup?.close();
					loginPopups.current[providerId] = null;
					setMessage(errorMessage(error));
			...
			(0, react.useEffect)(() => {
				const activePolls = Object.entries(pending).filter(([_, auth]) => auth.sessionId && auth.status === "pending" && !auth.authorizationCodeRequired && !auth.inputRequired);
				if (activePolls.length === 0) return;
				const timer = setInterval(() => {
					for (const [providerId, auth] of activePolls) {
						if (!auth.sessionId) continue;
						api.pollProviderLogin(providerId, auth.sessionId).then((response) => {
							setStatus(response.snapshot);
							setPending((value) => ({
								...value,
							...
							if (response.result.authorizationUrl && popup && !popup.closed) try {
								popup.location.replace(response.result.authorizationUrl);
							} catch {}
			...
			const submitCode = async (providerId) => {
				const auth = pending[providerId];
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				className: `dcs-provider-section${embedded ? " dcs-provider-embedded" : ""}`,
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
						className: "dcs-provider-header",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", { children: embedded ? "其他订阅 Provider" : "订阅 Provider" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: "Claude、Grok、Cursor 与 Antigravity 的 OAuth/官方会话。凭据仅保存在 Host。" })] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							disabled: busy !== null,
							onClick: () => void run("scan", () => api.scanProvider()),
							children: "扫描官方会话"
						})]
					}),
					message && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						role: "status",
						className: "dcs-provider-message",
						children: message
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dcs-provider-grid",
						children: (status?.providers ?? []).map((provider) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ProviderCard, {
							provider,
							busy,
							pending: pending[provider.providerId],
							code: codes[provider.providerId] ?? "",
							onCode: (code) => setCodes((value) => ({
								...value,
					...
					status && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", {
						className: "dcs-provider-storage",
						children: [
							"凭据存储：",
							status.storage.kind,
							" · ",
							status.storage.encrypted ? "已加密" : "所有者权限保护"
						]
					})
				]
			});
		...
		function ProviderCard(props) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("article", {
				className: "dcs-provider-card",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dcs-provider-card-head",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: provider.displayName }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: provider.providerId })] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							"data-live": provider.accounts.length > 0,
							children: provider.accounts.length > 0 ? "已连接" : "未登录"
						})]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dcs-provider-actions",
					...
					pending?.instructions && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: "dcs-provider-instructions",
						children: pending.instructions
					}),
					pending?.authorizationUrl && pending.status !== "completed" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dcs-provider-auth-link",
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
					...
					pending?.sessionId && pending.status !== "completed" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dcs-provider-auth-actions",
						children: pending.inputRequired || pending.authorizationCodeRequired ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							value: props.code,
							onChange: (event) => props.onCode(event.target.value),
							placeholder: "粘贴网页返回的授权码 (code#state)"
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							disabled: busy !== null || props.code.trim() === "",
					...
					(provider.candidates ?? []).filter((candidate) => !candidate.imported).map((candidate) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dcs-provider-account",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: candidate.displayName ?? candidate.email ?? candidate.accountId ?? "发现的官方会话" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: candidate.source ?? candidate.candidateId })] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							disabled: busy !== null,
							onClick: () => props.onImport(candidate.candidateId),
							children: "导入"
						})]
					}, candidate.candidateId)),
					provider.accounts.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: "dcs-provider-empty",
						children: "尚未导入账号。可先扫描本机官方会话，或直接发起浏览器 OAuth。"
					}) : provider.accounts.map((account) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dcs-provider-account",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: account.displayName ?? account.email ?? account.accountId }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: account.email ?? account.accountId })] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							disabled: busy !== null,
							onClick: () => props.onRemove(account.accountId),
							children: "移除"
						})]
					}, account.accountId))
				]
		...
		function CodexSubscriptionSection({ t }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ProviderSubscriptionsSection, { embedded: true }),
...
.dcs-provider-section{color:var(--dsw-alias-label-primary);padding:4px 0 20px}.dcs-provider-embedded{border-top:1px solid var(--dsw-alias-border-l2);padding:16px 0 0}.dcs-provider-embedded .dcs-provider-header h2{font-size:14px;font-weight:650}.dcs-provider-embedded .dcs-provider-header button,.dcs-provider-embedded .dcs-provider-actions button,.dcs-provider-embedded .dcs-provider-auth-actions button,.dcs-provider-embedded .dcs-provider-account button,.dcs-provider-embedded .dcs-provider-copy-btn{background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:999px;color:var(--dsw-alias-label-primary);font:inherit;font-size:13px;line-height:1;padding:8px 13px}.dcs-provider-embedded .dcs-provider-grid{display:block;margin-top:14px}.dcs-provider-embedded .dcs-provider-card{background:transparent;border:0;border-top:1px solid var(--dsw-alias-border-l2);border-radius:0;padding:15px 0}.dcs-provider-embedded .dcs-provider-card-head strong{font-size:13px}.dcs-provider-embedded .dcs-provider-card-head small{font-size:11px;margin-top:2px}.dcs-provider-embedded .dcs-provider-actions{justify-content:flex-end}.dcs-provider-embedded .dcs-provider-instructions,.dcs-provider-embedded .dcs-provider-empty{font-size:12px}.dcs-provider-embedded .dcs-provider-storage{border-top:1px solid var(--dsw-alias-border-l2);padding-top:12px}
.dcs-provider-header{align-items:flex-start;display:flex;gap:16px;justify-content:space-between}.dcs-provider-header h2{font-size:16px;margin:0 0 5px}.dcs-provider-header p{color:var(--dsw-alias-label-tertiary);font-size:12px;margin:0}.dcs-provider-header button,.dcs-provider-actions button,.dcs-provider-auth-actions button,.dcs-provider-account button{background:transparent;border:1px solid var(--dsw-alias-border-l2);border-radius:7px;color:inherit;cursor:pointer;font-size:12px;padding:7px 10px}.dcs-provider-header button:disabled,.dcs-provider-actions button:disabled,.dcs-provider-auth-actions button:disabled,.dcs-provider-account button:disabled{cursor:default;opacity:.5}
.dcs-provider-message{background:var(--dsw-alias-bg-layer-2);border-radius:7px;font-size:12px;margin:12px 0;padding:9px 11px}.dcs-provider-grid{display:grid;gap:10px;grid-template-columns:repeat(2,minmax(0,1fr));margin-top:14px}.dcs-provider-card{background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:10px;padding:13px}.dcs-provider-card-head{align-items:flex-start;display:flex;gap:10px;justify-content:space-between}.dcs-provider-card-head div,.dcs-provider-account div{display:flex;min-width:0;flex-direction:column}.dcs-provider-card-head small,.dcs-provider-account small{color:var(--dsw-alias-label-tertiary);font-size:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.dcs-provider-card-head span{color:var(--dsw-alias-label-tertiary);font-size:11px}.dcs-provider-card-head span[data-live=true]{color:var(--dsw-alias-label-success,#39a779)}.dcs-provider-actions,.dcs-provider-auth-actions{display:flex;flex-wrap:wrap;gap:7px;margin-top:11px}.dcs-provider-auth-actions input{background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:7px;color:inherit;flex:1 1 180px;min-width:0;padding:7px 9px}.dcs-provider-instructions,.dcs-provider-empty{color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:1.5;margin:10px 0 0}.dcs-provider-account{align-items:center;border-top:1px solid var(--dsw-alias-border-l1);display:flex;gap:10px;justify-content:space-between;margin-top:10px;padding-top:10px}.dcs-provider-auth-link{margin:10px 0 6px}.dcs-provider-auth-btn{background:var(--dsw-alias-color-primary,#4b89dc);border-radius:7px;color:#fff!important;display:inline-block;flex:1 1 auto;font-size:12px;font-weight:500;padding:8px 12px;text-align:center;text-decoration:none!important;box-sizing:border-box}.dcs-provider-auth-btn:hover{opacity:.9}.dcs-provider-copy-btn{background:transparent;border:1px solid var(--dsw-alias-border-l2);border-radius:7px;color:inherit;cursor:pointer;flex:0 0 auto;font-size:12px;padding:8px 10px;white-space:nowrap}.dcs-provider-copy-btn:hover{background:var(--dsw-alias-bg-layer-2)}.dcs-provider-storage{color:var(--dsw-alias-label-tertiary);font-size:11px;margin:12px 0 0}
@media(max-width:700px){.dcs-provider-grid{grid-template-columns:1fr}.dcs-provider-header{flex-direction:column}}
