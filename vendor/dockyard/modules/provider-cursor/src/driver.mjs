...
const CURSOR_ALLOWED_ORIGINS = new Set(["https://cursor.com", "https://api2.cursor.sh"]);

function validateCursorOrigin(value, label) {
  const url = new URL(value);
  if (url.protocol !== "https:" || !CURSOR_ALLOWED_ORIGINS.has(url.origin)) {
    throw new Error(`Cursor ${label} endpoint must use an allowlisted official HTTPS origin`);
  }
...
function validateCursorUrl(value, label, expectedOrigin = null) {
  ...
  if (expectedOrigin && url.origin !== new URL(expectedOrigin).origin) {
    throw new Error(`Cursor ${label} endpoint must match the official API origin`);
  }
  return url;
}
...
export class CursorSubscriptionDriver {
  constructor({
    ...
    fetchImpl = fetch,
    allowDesktopSessionImport = false,
  } = {}) {
  ...
  #readDesktopSession() {
    const session = readCursorDesktopSession({
      env: this.env, home: this.home, allowDesktopSession: this.allowDesktopSessionImport,
    });
  ...
  async dispose() {
    const authorizers = new Set([this.oauthAuthorizer, this.browserAuthorizer, this.cliAuthorizer, this.clientSessionAuthorizer]);
