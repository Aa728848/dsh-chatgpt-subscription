...
const CLAUDE_ALLOWED_ORIGINS = new Set(["https://claude.com", "https://platform.claude.com"]);

function validateClaudeEndpoint(value, label, expectedOrigin) {
  const url = new URL(value);
  if (url.protocol !== "https:" || !CLAUDE_ALLOWED_ORIGINS.has(url.origin) || (expectedOrigin && url.origin !== expectedOrigin)) {
    throw new Error(`Claude ${label} endpoint must use its allowlisted HTTPS origin`);
  }
