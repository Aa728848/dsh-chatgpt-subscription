...
export function createGrokNativeExecutor({
  ...
  const executor = async ({ request = {}, credential, context = {} } = {}) => {
    ...
    // This provider represents an imported subscription OAuth account. API-key
    // environment variables must not silently masquerade as that account.
    const token = firstString(credential?.access);
    if (!token) {
