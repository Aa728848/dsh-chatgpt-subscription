...
const GROK_ALLOWED_ORIGINS = new Set(["https://auth.x.ai", "https://cli-chat-proxy.grok.com"]);

function validateGrokEndpoint(value, label, expectedOrigin) {
  const url = new URL(value);
  if (url.protocol !== "https:" || !GROK_ALLOWED_ORIGINS.has(url.origin) || (expectedOrigin && url.origin !== expectedOrigin)) {
    throw new Error(`Grok ${label} endpoint must use its allowlisted HTTPS origin`);
  }
...
export class GrokOAuthDriver {
  ...
  async dispose() {
    const authorizers = new Set([this.oauthAuthorizer, this.browserAuthorizer, this.cliAuthorizer]);
