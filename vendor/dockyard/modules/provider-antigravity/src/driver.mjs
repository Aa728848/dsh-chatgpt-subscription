...
const ANTIGRAVITY_ALLOWED_ORIGINS = new Set(["https://accounts.google.com", "https://oauth2.googleapis.com", "https://www.googleapis.com"]);

function validateAntigravityEndpoint(value, label, expectedOrigin) {
  const url = new URL(value);
  if (url.protocol !== "https:" || !ANTIGRAVITY_ALLOWED_ORIGINS.has(url.origin) || (expectedOrigin && url.origin !== expectedOrigin)) {
    throw new Error(`Antigravity ${label} endpoint must use its allowlisted HTTPS origin`);
  }
...
function validateAntigravityRedirect(value) {
  ...
  if (url.protocol !== "http:" || !["localhost", "127.0.0.1", "::1"].includes(url.hostname.replace(/^\[|\]$/g, ""))) {
    throw new Error("Antigravity OAuth redirect must use loopback HTTP");
  }
  return url.toString();
}
...
export function createAntigravityOAuthAuthorizer({
  ...
  return Object.freeze({ begin, poll, cancel, submitAuthorizationCode, dispose });
}
...
export class AntigravityOfficialSessionDriver {
  ...
  async dispose() {
    const authorizers = new Set([this.oauthAuthorizer, this.browserAuthorizer, this.cliOAuthAuthorizer]);
  ...
  async refreshAccount(account, context = {}) {
    ...
    // Captured/browser accounts are isolated credentials and must never fall
    // back to the process-wide CLI. An active CLI session, however, owns its
    // credentials internally and intentionally uses official slash commands.
    if (typeof this.quotaReader === "function" && account?.auth?.credentialRef
      && account?.resources?.sessionPersistence !== "active") {
