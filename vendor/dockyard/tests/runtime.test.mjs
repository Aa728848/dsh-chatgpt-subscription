...
test("runtime keeps the secure secret store in invocation context", async () => {
  const home = await mkdtemp(join(tmpdir(), "dockyard-runtime-invoke-secret-"));
  const secretStore = new MemorySecretStore();
  let seen = null;
  const module = {
    ...fakeModule(),
    async startAuthorization() { return { status: "completed", accounts: [{ providerId: "test-provider", accountId: "account", credentialRef: "keychain://account", auth: { kind: "oauth", credentialRef: "keychain://account", scopes: [] } }] }; },
    async invoke(_request, _invocation, context) { seen = context.secretStore; return { ok: true }; },
  };
  const runtime = new DockyardRuntime({ providers: [{ module }], runtime: new ModuleRuntime({ logger: { error() {}, warn() {}, info() {} } }), stateStore: new JsonStateStore({ home }), secretStore });
  try {
    await runtime.startAuthorization("test-provider");
    await runtime.invoke("test-provider", {});
    assert.equal(seen, secretStore);
  } finally { await runtime.dispose(); await rm(home, { recursive: true, force: true }); }
});

test("runtime rolls back partial initialization and can retry cleanly", async () => {
  const home = await mkdtemp(join(tmpdir(), "dockyard-runtime-init-rollback-"));
  let failSecond = true;
  const first = fakeModule();
  const second = { ...fakeModule(), manifest: { id: "second-provider", kind: "provider", displayName: "Second" }, async activate(context) { if (failSecond) throw new Error("activation failed"); context.registerService("provider:second-provider", this); } };
  const runtime = new ModuleRuntime({ logger: { error() {}, warn() {}, info() {} } });
  const dockyard = new DockyardRuntime({ providers: [{ module: first }, { module: second }], runtime, stateStore: new JsonStateStore({ home }), secretStore: new MemorySecretStore() });
  try {
    await assert.rejects(() => dockyard.init(), /activation failed/);
    assert.equal(runtime.has("test-provider"), false);
    assert.equal(runtime.has("second-provider"), false);
    assert.deepEqual(dockyard.snapshot().providers, []);
    failSecond = false;
    await dockyard.init();
    assert.deepEqual(dockyard.snapshot().providers.map((provider) => provider.providerId), ["test-provider", "second-provider"]);
  } finally { await dockyard.dispose(); await rm(home, { recursive: true, force: true }); }
});
...
test("runtime persists accounts completed by manual OAuth code submission", async () => {
  const home = await mkdtemp(join(tmpdir(), "dockyard-runtime-manual-code-"));
  try {
    const module = {
      manifest: { id: "test-provider", kind: "provider", displayName: "Test provider" },
      async activate(context) { context.registerService("provider:test-provider", this); },
      async discover() { return { candidates: [] }; },
      async submitAuthorizationCode() { return { status: "completed", accounts: [{ providerId: "test-provider", accountId: "manual-account", credentialRef: "keychain://manual-account", auth: { kind: "oauth", credentialRef: "keychain://manual-account", scopes: [] } }] }; },
      async refreshAccount() { return {}; }, async getQuota() { return {}; }, async getCatalog() { return { models: [] }; }, async invoke() { return {}; },
    };
    const runtime = new DockyardRuntime({ providers: [{ module }], runtime: new ModuleRuntime({ logger: { error() {}, warn() {}, info() {} } }), stateStore: new JsonStateStore({ home }), secretStore: new MemorySecretStore() });
    await runtime.submitAuthorizationCode("test-provider", "test-provider:session", "code#state");
    assert.equal(runtime.snapshot().providers[0].accounts[0].accountId, "manual-account");
    const reloaded = new DockyardRuntime({ providers: [{ module }], runtime: new ModuleRuntime({ logger: { error() {}, warn() {}, info() {} } }), stateStore: new JsonStateStore({ home }), secretStore: new MemorySecretStore() });
    await reloaded.init();
    assert.equal(reloaded.snapshot().providers[0].accounts[0].accountId, "manual-account");
