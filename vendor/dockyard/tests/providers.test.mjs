...
test("browser OAuth rejects callbacks and pasted codes without matching state", async () => {
  ...
  const callbackAuthorizer = createBrowserOAuthAuthorizer({
    ...
    exchangeCode: async ({ code }) => { if (code !== "valid-code") throw new Error("must not exchange an invalid callback"); return { accessToken: "valid" }; },
    importCredentials: async () => [{ providerId: "test-browser-state-provider", accountId: "valid-account" }],
  });
  ...
  assert.equal(callbackResult.status, "pending");
  const validCallback = new URL(callbackRequest.redirectUri);
  validCallback.searchParams.set("code", "valid-code");
  validCallback.searchParams.set("state", callbackRequest.state);
  assert.equal((await fetch(validCallback)).status, 200);
  const completedAfterProbe = await callbackAuthorizer.poll(callbackStarted.sessionId);
  assert.equal(completedAfterProbe.status, "completed");
